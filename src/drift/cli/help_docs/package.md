...
4.  **`Dynamic Python Hook`**: A `drift_package.py` (or custom `hook_file`) defining `configure_package(context: PackageHookContext)` to dynamically transform package settings based on host facts, workspace context, and environment. The hook executes with **zero footprint on `os.environ`**, receiving all secrets, facts, and environment variables in-memory via `context.env`. This is the **recommended place to fetch package-specific configuration or secrets from remote servers** and inject them into `[env.default]`, `[env.secrets]`, or `[env.override]`.
