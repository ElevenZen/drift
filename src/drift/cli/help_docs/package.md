...
4.  **`Lifecycle Hooks`**: Shell command hooks executed atomically during source generation, render, installation, update, uninstallation, and health probe sequences 
    ...
    If `sudo = true`, installation, update, and uninstallation hooks (`pre/post_install`, `pre/post_update`, `pre/post_uninstall`) run with `sudo` elevation, while source, render, and health probe hooks (`pre_source`, `post_render`, `health`) always run in user space without `sudo`.
