...
| **Context & Fact Binding** | Positional args (`$1` action, `$2` version) | Positional arg (`$1` instance count) | Target paths streamed via `stdin` | Automatically injected `$drift_*` host facts & scoped `[env]` overrides |
