...
*   [`resolve_and_interpolate_workspace_config(data, secrets_file=None) -> Dict[str, Any]`](../src/drift/config/workspace_config.py): Pure in-memory topological DAG resolution for workspace `[env.secrets]` and `[env.default]`, returning fully interpolated dictionary.
